...
def start_update_tbl(table_name):
    ...
    if table_name == "src_demands_gr":
        # Создаём DataFrame
        print('Соответствие таблице MySQL: ', table_name)
        txt_sql = """
            ВЫБРАТЬ
                МесячныеДанные.Год,
                МесячныеДанные.Месяц,
                МесячныеДанные.Количество КАК КоличествоПоМесяцу,
                МесячныеДанные.Сумма КАК СуммаПоМесяцу,
                ГодовыеДанные.Количество КАК КоличествоПоГоду,
                ГодовыеДанные.Сумма КАК СуммаПоГоду
            ИЗ
                (ВЫБРАТЬ
                    ГОД(Реализация.Дата) КАК Год,
            ...
            (ВЫБРАТЬ
                ...
                ПО МесячныеДанные.Год = ГодовыеДанные.Год 
            УПОРЯДОЧИТЬ ПО
                МесячныеДанные.Год ВОЗР,
                МесячныеДанные.Месяц ВОЗР
        """
        fields = ["Год","Месяц","КоличествоПоМесяцу","СуммаПоМесяцу","КоличествоПоГоду","СуммаПоГоду"]
        data = getOne(txt_sql, fields)
        print(data)
        df = pd.DataFrame(data)
